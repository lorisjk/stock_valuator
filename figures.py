...
def plot_metric(ax, metrics_long, ticker, concept, ylabel, ref_line=None, percent=False, symlog=False, show_mean=False) -> None:
    ...
    if show_mean and not filtered.empty:
        mean_value = filtered["value"].mean()
        label = f"Ø {mean_value:.2%}" if percent else f"Ø {mean_value:.1f}"
        ax.axhline(mean_value, color="red", linestyle="solid", linewidth=1, label=label)
...
def plot_valuation(ticker: str, valuation_history: pd.DataFrame, output_path: str, years: int = 5) -> None:
    ...
    for ax, (concept, ylabel, ref_line, percent) in zip(axes_flat, concepts_to_plot):
        plot_metric(ax, filtered, ticker, concept, ylabel, ref_line, percent, show_mean=True)

    fig.suptitle(f"Valuationsdaten {ticker}")
    fig.tight_layout()
    fig.savefig(output_path)
    plt.close(fig)