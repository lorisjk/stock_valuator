from fetchers.edgar import extract_annual_values, extract_summed_annual_values
import pandas as pd

def extract_merged_annual_values(us_gaap_data: dict, candidate_tags: list[str], is_point_in_time: bool = False) -> list[dict]:
    merged = {}
...
def build_dataframe(ticker: str, company_info: dict, concept_candidates: dict) -> pd.DataFrame:
    ...
    for key, cfg in concept_candidates.items():
        ...
        if mode == "sum":
            ...
            values = extract_merged_annual_values(
                company_info["facts"]["us-gaap"],
                cfg["tags"],
                is_point_in_time=cfg["point_in_time"]
            )
        ...
        for v in values:
            rows.append({"ticker": ticker, "concept": key, "end": v["end"], "value": v["value"]})
