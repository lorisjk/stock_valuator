...
def extract_period_values(concept_data: dict, is_point_in_time: bool = False, period: str = "annual") -> list[dict]:
    ...
    Gibt für period="quarterly" zusätzlich "fp" und "fy" pro Eintrag zurück,
    weil diese für die Q4-Ableitung (siehe derive_q4_values) gebraucht werden.
    """
    values = {}
    ...
    for item in items:
        ...
        if period == "annual":
            ...
            if is_point_in_time:
                # Bilanzwerte: jeder Quartalsstichtag zaehlt. "FY" ist hier
                # der Stichtag zum Fiskaljahresende = identisch mit dem
                # Q4-Stichtag, muss also NICHT abgeleitet werden.
                is_valid = fp in ("Q1", "Q2", "Q3", "FY")
            else:
                if "start" not in item:
                    continue
                start = date.fromisoformat(item["start"])
                ...
                days_diff = (end - start).days
                if fp in ("Q1", "Q2", "Q3"):
                    is_valid = 80 <= days_diff <= 100
                elif fp == "FY":
                    # Der volle Jahreswert wird hier NUR als Zutat fuer die
                    # spaetere Q4-Ableitung mitgenommen (Q4 = FY - Q1-Q2-Q3),
                    # nicht als eigener Quartals-Datenpunkt.
                    is_valid = 350 <= days_diff <= 380
                else:
                    is_valid = False
        else:
...
def derive_q4_values(quarterly_and_fy_values: list[dict]) -> list[dict]:
    """
    Leitet Q4 rechnerisch ab, weil SEC-Filer Q4 so gut wie nie separat melden:
    Q4 = FY-Gesamtwert - (Q1 + Q2 + Q3)
 
    Gruppierung erfolgt ueber das Fiscal Year ("fy"-Feld), NICHT ueber das
    end-Datum, weil Q1/Q2/Q3/FY jeweils unterschiedliche end-Daten haben.
    """
    by_fy = {}
    for v in quarterly_and_fy_values:
        fy = v.get("fy")
        by_fy.setdefault(fy, {})[v["fp"]] = v
 
    result = []
    for fy, entries in by_fy.items():
        for fp in ("Q1", "Q2", "Q3"):
            if fp in entries:
                e = entries[fp]
                result.append({"end": e["end"], "value": e["value"], "filed": e["filed"]})
 
        # Q4 nur ableitbar, wenn FY UND alle drei Quartale fuer dasselbe fy vorliegen
        if "FY" in entries and all(q in entries for q in ("Q1", "Q2", "Q3")):
            fy_entry = entries["FY"]
            q1q2q3_sum = sum(entries[q]["value"] for q in ("Q1", "Q2", "Q3"))
            q4_value = fy_entry["value"] - q1q2q3_sum
            result.append({
                "end": fy_entry["end"],  # Fiskaljahresende = Ende von Q4
                "value": q4_value,
                "filed": fy_entry["filed"],
            })
...
def extract_quarterly_values(concept_data: dict, is_point_in_time: bool = False) -> list[dict]:
    ...
    Bei Stichtagswerten ist keine Ableitung noetig (FY-Stichtag = Q4-Stichtag).
    """
